...
const ListRoom = () => {
  ...
  const fetchRooms = async () =>{
    try {
      const {data} = await axios.get('/api/rooms/owner', {headers: {Authorization: `Bearer ${await gaetToken()}`}})
      if(data.success){
        setRooms(data.rooms)
      } else {
  ...
  const toggleAvailability = async (roomId) => {
    const {data} = await axios.post('/api/rooms/toggle-availability', {roomId}, {headers: {Authorization: `Bearer ${await gaetToken()}`}})
    if(data.success) {
